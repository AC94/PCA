####### PRINCIPAL COMPONENT ANALYSIS FOR ASSET ALLOCATION: AN APPLICATION TO THE UK MARKET #######
## Alberto Ciampini - alberto.ciampini@aol.com - BSc Economics @ Università Cattolica del Sacro Cuore, Milan ##
...
tickers<-c("BLT.L","RIO.L","VOD.L","BT-A.L","REL.L","CCL.L","ULVR.L","BATS.L","RDSA.L","BP.L","HSBA.L","LLOY.L","GSK.L","AZN.L","BA.L","EXPN.L","LAND.L","BLND.L","ARM.L","SGE.L","NG.L","SSE.L")
...
chart.CumReturns(data,geometric=TRUE,colorset=rainbow(22),legend="topleft",main="Portfolio Constituents Cumulative Returns",ylab="Cumulative return (1£)",begin="1",wealth.index=TRUE,cex.lab=TRUE)
chart.Boxplot(data,colorset=rainbow(22),legend="topleft",main="Boxplot of Portfolio Constituents Returns")
chart.RiskReturnScatter(data,colorset=rainbow(22),method="calc",Rf=.0,main="Portfolio Relative Risk and Return",add.boxplots=TRUE,scale=scale,add.sharpe=c(1,2,3))
...
plot(cumsum(pca.corr$sd^2/sum(pca.corr$sd^2)),main="Plot of Cumulative Variance Explained by Principal Compoents",type="l",ylab="Cumulative Variance",xlab="Component Number",col="red")
...
sectors<-c("Basic Materials","Basic Materials","Communication Services","Communication Services","Consumer Cyclical","Consumer Cyclical","Consumer Defensive","Consumer Defensive","Energy","Energy","Financial Services","Financial Services","Healthcare","Healthcare","Industrials","Industrials","Real Estate","Real Estate","Technology","Technology","Utilities","Utilities")
...
chart.RiskReturnScatter(PPret,colorset=palette,Rf=.0,method="calc",main="Principal Portfolios Relative Risk and Return",add.boxplots=TRUE,scale=scale,add.sharpe=c(1,2,3))
chart.RiskReturnScatter(PPret[,1:n],colorset=palette,Rf=.0,method="calc",main="Retained Principal Portfolios Relative Risk and Return",add.boxplots=TRUE,scale=scale,add.sharpe=c(1,2,3))
chart.Boxplot(PPret[,1:n],colorset=palette,legend="topleft",main="Boxplot of Retained Principal Portfolios Returns") 

#####performance of PPs#####
chart.CumReturns(PPret[,1:n],geometric=TRUE,colorset=palette,legend="topleft",main="Retained Principal Portfolios Cumulative Returns",ylab="Cumulative return (1£)",begin="1",wealth.index=TRUE,cex.lab=TRUE)
...
chart.RelativePerformance(PPret[,1:n],Pr,colorset=palette,legend="topleft",ylab="Cumulative return (1£)",main="Relative Performance of the Retained Principal Portfolios aginst the 1/N Portfolio")
#The PPs that exhibited a net positive absolute return (relative to the Benchmark) for the whole study period are PPn1, PPn2;we now evaluate their performances only
n1=5
n2=6
newdata<-cbind(PPret[,n1],PPret[,n2])
chart.RelativePerformance(newdata,Pr,colorset=palette,legend="topleft",ylab="Cumulative return (1£)",main="Relative Performance of the PP5 and PP6 aginst the 1/N Portfolio")
newdataBench<-merge(newdata,Pr)
chart.CumReturns(newdataBench,colorset=palette,legend="topleft",main="PP5, PP6 and 1/N Portfolio Cumulative Returns",ylab="Cumulative return (1£)",begin="1",wealth.index=TRUE,cex.lab=TRUE)
